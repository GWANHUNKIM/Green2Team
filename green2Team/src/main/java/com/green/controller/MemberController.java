...
public class MemberController {
	...
	public String main(Model model) {
		model.addAttribute("vrLists", vrBoardService.getVrList());
		model.addAttribute("videoLists",videoBoardService.getVideoList());
		return "/home/main";
	}
	...
	public String newMember(RegisterCommand registerCommand) {
		try {
			...
			return "redirect:/home/newMember";
		}
	}

	@GetMapping("/login")
	public String login(Model model, @CookieValue(value = "rememberId", required = false) Cookie c) {
	...
	public String login(LoginCommand loginCommand, HttpSession session, HttpServletResponse response) {
		try {
			LoginCommand info = service.login(loginCommand.getId(), loginCommand.getPw());
